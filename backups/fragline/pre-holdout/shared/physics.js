...
// Ramps are wedges: a box whose top rises linearly along ramp.axis (0 = x, 2 = z) toward ramp.dir.
// Ray vs wedge by clipping against its 6 planes (n·p <= c inside).
function rayRamp(o, d, b) {
  ...
  const planes = [
    ...
    [[0, -1, 0], -b.min[1]],
    [top, b.min[1] + slope * (dir > 0 ? -b.min[axis] : b.max[axis])],
  ];
...
// Ray vs a player's hitboxes. pl = { x, y, z, yaw, c }. Returns { t, part } (closest) or null.
export function rayPlayer(o, d, maxDist, pl) {
  const cy = Math.cos(pl.yaw), sy = Math.sin(pl.yaw);
  const ox = o[0] - pl.x, oz = o[2] - pl.z;
  const lo = [ox * cy - oz * sy, o[1] - pl.y, ox * sy + oz * cy];
  ...
  for (const hb of hitboxes(pl.c || 0)) {
    const r = rayBox(lo, ld,
      ...
      [hb.c[0] + hb.h[0], hb.c[1] + hb.h[1], hb.c[2] + hb.h[2]]);
    if (r && r.t <= maxDist && (!best || r.t < best.t)) best = { t: r.t, part: hb.part };
  }
  return best;
}

// Full bullet trace with wall penetration. Returns
// { endT, player: { part, pen } | null, impacts: [{ t, n, mat, exit }] }
...
export function traceBullet(o, d, maxDist, boxes, target, wallPen) {
  ...
  const ph = target ? rayPlayer(o, d, maxDist, target) : null;
  let pen = 1;
  const impacts = [];
  for (const h of walls) {
    if (ph && h.t > ph.t) break;
    impacts.push({ t: h.t, n: h.n, mat: h.box.mat });
  ...
  if (ph) return { endT: ph.t, player: { part: ph.part, pen }, impacts };
...
function overlapsBox(x, y, z, h, b) {
  ...
  return x + r > b.min[0] && x - r < b.max[0] && z + r > b.min[2] && z - r < b.max[2] &&
    y + h > b.min[1] && y < topAt(b, x, z) - 1e-6;
}
  ...
  for (let s = 0; s < n; s++) {
    for (const a of [0, 2]) {
      ...
      for (const b of boxes) {
        ...
        const up = top - pos[1];
        // step onto stairs/low ledges; a ramp's slope lifts you even mid-air (you're touching it)
        if ((wasGrounded || grounded || b.ramp) && up > 0 && up <= P.step && !blocked(pos[0], top + 0.001, pos[2], h, boxes)) {
          pos[1] = top;
          if (b.ramp && vel[1] < 0) vel[1] = 0;
          continue;
        }
    ...
    for (const b of boxes) {
      ...
      } else if (dy > 0 && pos[1] + h - b.min[1] <= dy + 0.05) {
        pos[1] = b.min[1] - h - 1e-4; vel[1] = 0;
      }
    }
  }
