...
import { SPAWNS, BUY_RADIUS } from '../shared/map.js';
import { modeOf } from '../shared/modes.js';
import { LAKE_SPAWNS } from '../shared/lake.js';
...
export class Room {
  constructor(code, opts = {}) {
    ...
    this.spawns = this.mode.map === 'lake' ? LAKE_SPAWNS : SPAWNS;
    this.vsBot = !!opts.bot;
  ...
  makePlayer(name, ws, bot = false) {
    ...
    return {
      id: bot ? 'bot' : 'p' + uid++, name, ws, bot, side,
  ...
  handle(p, m) {
    switch (m.t) {
      case 'st': return this.onState(p, m);
      ...
      case 'ping':
        p.ping = m.ping | 0;
        return this.send(p, { t: 'pong', ts: m.ts });
      case 'chat':
        return this.broadcast({ t: 'chat', name: p.name, text: String(m.text || '').slice(0, 120) });
    }
  }
  ...
  onState(p, m) {
    ...
    p.st = {
      ...
      c: Math.min(1, Math.max(0, +m.c || 0)), w: WEAPONS[m.w] ? m.w : 'knife', g: !!m.g,
    };
