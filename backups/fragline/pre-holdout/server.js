...
wss.on('connection', ws => {
  ...
  ws.on('message', raw => {
    ...
    if (m.mode === 'join') {
      ...
      room = new Room(newCode(), { bot: m.mode === 'bot', diff: m.diff, gameMode: m.mode === 'create' ? m.gameMode : 'classic' });
      rooms.set(room.code, room);
    }
    player = room.addPlayer(ws, name);
    console.log(`[${room.code}] ${name} joined (${room.players.length}/2${room.vsBot ? ', vs bot' : ''})`);
  });
