...
export class Weapons {
  constructor(game) {
    this.g = game;
    this.slots = { 1: null, 2: null, 3: { w: 'knife', uid: 'knife' } };
    this.ammo = {};
    this.slot = 3;
    this.lastSlot = 2;
    this.reset();
  ...
  get cur() { return this.slots[this.slot] || this.slots[3]; }
  ...
  // Server inventory update. New uids = freshly bought/issued weapons with full ammo.
  setInventory(s1, s2) {
    let fresh = 0;
    for (const [k, it] of [[1, s1], [2, s2]]) {
      const prev = this.slots[k];
      this.slots[k] = it || null;
      if (it && !(it.uid in this.ammo)) this.ammo[it.uid] = { mag: WEAPONS[it.w].mag, reserve: WEAPONS[it.w].reserve };
      if (it && prev?.uid !== it.uid && !fresh) fresh = k;
    }
    if (fresh && this.g.player.alive) this.equip(fresh, true);
    else if (!this.slots[this.slot]) this.equip(this.slots[1] ? 1 : this.slots[2] ? 2 : 3, true);
  }
  ...
  onSpawn(afterDeath) {
    ...
    else this.equip(this.slots[1] ? 1 : this.slots[2] ? 2 : 3, true);
  }

  refill() {
    for (const s of [1, 2]) {
      const it = this.slots[s];
      if (it) this.ammo[it.uid] = { mag: WEAPONS[it.w].mag, reserve: WEAPONS[it.w].reserve };
    }
  }
  ...
  cycle(dir) {
    const order = [1, 2, 3].filter(s => this.slots[s]);
  ...
  reload() {
    ...
    if (!a || w.cat === 'melee' || this.reloadEnd || a.mag >= w.mag || a.reserve <= 0 || now < this.deployEnd) return;
    this.scope = this.rezoom = 0;
    this.inspectStart = -1;
  ...
  update(dt, input) {
    ...
    if (w.shellReload && this.reloadEnd && now >= this.shellNext) {
      ...
      if (a.mag >= w.mag || a.reserve <= 0) { this.reloadEnd = 0; g.sound.play('rl_pump', { vol: 0.9, delay: 0.05 }); }
    }
    if (this.rezoom && now >= this.rezoomAt && !this.reloadEnd) { this.scope = this.rezoom; this.rezoom = 0; }

    if (!input.fire) this.triggerDown = false;
    if (!g.player.alive || !g.canShoot() || now < this.deployEnd) return;
  ...
  fire() {
    ...
    const jit = i >= 2 ? w.jitter : 0;
    const kick = [next[0] - cur[0] + (Math.random() - 0.5) * jit, next[1] - cur[1] + (Math.random() - 0.5) * jit * 0.5];
    this.sprayIdx = i + 1;

    const inacc = this.inaccuracy();
    const baseYaw = pl.yaw - this.punch[0] * DEG, basePitch = pl.pitch + this.punch[1] * DEG;
    const eye = pl.eye, target = g.remoteTarget();
    ...
    for (let k = 0; k < w.pellets; k++) {
      ...
      const end = at(tr.endT);
      for (const im of tr.impacts) {
        g.world.impact(at(im.t), im.n, im.mat);
        if (!heard && !im.exit) { g.sound.play('impact' + (im.mat === 'w' ? '_w' : im.mat === 'm' ? '_m' : ''), { pos: at(im.t), vol: 0.35, ref: 2 }); heard = true; }
      }
      if (tr.player) {
        hits.push({ part: tr.player.part, pen: r3(tr.player.pen) });
        g.world.blood(end, d, tr.player.part === 'head');
      }
      dirs.push(d.map(r3));
      ends.push(end.map(r3));
    }
    if (w.acc) this.bloom = Math.min(w.acc.fireMax, this.bloom + w.acc.fire);
    this.punch[0] += kick[0];
    this.punch[1] += kick[1];
    this.lastShot = now;

    g.sound.play('shot_' + w.id, { vol: 0.85 });
    g.vm.fire(w.cat === 'pistol' ? (w.id === 'deagle' ? 0.8 : 0.45) : w.cat === 'sniper' || w.cat === 'shotgun' ? 1.3 : 1);
    const muzzle = g.muzzleWorld();
    ends.forEach((e, k) => { if (k < 3) g.world.tracer(muzzle, e, 0xfff0b8); });
    g.net.send({ t: 'shot', w: w.id, o: eye.map(r3), d: dirs, e: ends, h: hits });
    ...
    if (a.mag === 0 && a.reserve > 0) setTimeout(() => { if (this.clip === a && a.mag === 0) this.reload(); }, 250);
  }

  knife(alt) {
    const g = this.g, pl = g.player, w = this.w;
    this.nextFire = g.now + (alt ? 1.0 : 0.4);
    this.inspectStart = -1;
    g.vm.knife(alt);
    const eye = pl.eye, target = g.remoteTarget();
    let hit = null;
    for (const [dy, dp] of [[0, 0], [0.09, 0], [-0.09, 0], [0, 0.09], [0, -0.12]]) {
      ...
      if (tr.player) { hit = { part: tr.player.part, pen: 1 }; g.world.blood(eye.map((v, j) => v + d[j] * tr.endT), d, false); break; }
    }
    g.sound.play(hit ? 'knife_hit' : 'knife_swing', { vol: 0.8 });
