...
const last = {};
const setText = (id, v) => { v = String(v); if (last[id] !== v) { last[id] = v; $(id).textContent = v; } };
const setHTML = (id, v) => { const k = id + '#html'; if (last[k] !== v) { last[k] = v; $(id).innerHTML = v; } };
const setClass = (id, cls, on) => { const k = id + '.' + cls; if (last[k] !== on) { last[k] = on; $(id).classList.toggle(cls, on); } };
const setStyle = (id, prop, v) => { const k = id + ':' + prop; if (last[k] !== v) { last[k] = v; $(id).style[prop] = v; } };
const setHidden = (id, h) => { const k = id + '!hidden'; if (last[k] !== h) { last[k] = h; $(id).hidden = h; } };
...
export class Hud {
  ...
  listen(id, i, btn) {
    ...
    this.requestKey(code => {
      if (code !== 'Escape') {
        ...
        if (val) for (const a in binds) binds[a] = binds[a].map(c => (c === val ? '' : c)); // one key, one action
        binds[id][i] = val;
        this.save();
        this.onSettings(this.settings);
      }
  ...
  vitals(hp, armor, helmet) {
    ...
    setText('armor', armor);
    setText('armorLabel', helmet ? 'ARMOR + HELMET' : 'ARMOR');
  ...
  ammo(w, clip, slots, activeSlot) {
    setText('weaponName', (w.skin ? `${w.name} | ${w.skin}` : w.name).toUpperCase());
    setStyle('weaponName', 'color', RARITY[w.rarity] || '');
  ...
  renderBuy(st) {
    ...
    $('buyGrid').innerHTML = BUY_MENU.map(([cat, ids]) => `<div class="buyCol"><h3>${cat.toUpperCase()}</h3>${ids.map(id => {
