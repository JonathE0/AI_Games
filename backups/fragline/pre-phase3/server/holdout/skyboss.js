...
export class SkyBoss {
  ...
  spawn(now = Date.now()) {
    const hp = Math.round(SKY.pointHp * (1 + 0.5 * (this.room.activeCount() - 1)));
    this.state = { t0: now, hp: SKY_POINTS.map(() => hp), max: hp, nextBomb: now + 7000, nextMinions: now + 20000, dead: false };
  ...
  update(now) {
    ...
    if (now >= s.nextBomb) {
      ...
      const players = room.players.filter(p => p.alive && !p.downed), pieces = [...room.pieces.values()];
  ...
  hit(p, i, o, d, dmg, now = Date.now()) {
    ...
    let ok = false;
    for (let k = 0; k <= 4 && !ok; k++) ok = raySphere(o, u, skyPoint(t - k * 0.1, i), SKY.pointR + 0.8) >= 0;
    if (!ok) return false;
  ...
  die(p) {
    ...
    s.dead = true;
    for (const q of room.players) { q.money = Math.min(16000, q.money + SKY.reward); room.sendInv(q); }
    const a = Math.atan2(tr.z, tr.x), at = [room.map.core.x + Math.cos(a) * 9, 0, room.map.core.z + Math.sin(a) * 9];
    room.inventory.scatter(rollLoot('boss'), at, 2.4);
