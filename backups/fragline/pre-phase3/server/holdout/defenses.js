...
import { OUTPOST_BOXES } from '../../shared/outpost.js';
...
export class Defenses {
  ...
  onPlace(p, m) {
    ...
    if (!it || it.kind !== 'trap' || !(p.items[m.item] > 0) || !p.alive || p.downed) return;
    ...
    if (it.mount === 'floor' || it.mount === 'wall') {
      ...
      if (OUTPOST_BOXES.some(b => b.mat !== 'f' && overlaps(foot, b)) || room.aliveNodeBoxes().some(b => overlaps(foot, b))) return deny('Blocked');
    ...
    Object.assign(d, { id: this.nextId++, type: m.item, owner: p.id, uses: def.uses ?? 0, ammo: def.ammo ?? 0, next: 0, until: def.life ? Date.now() + def.life * 1000 : 0 });
  ...
  update(dt, now) {
    ...
    for (const d of [...this.list.values()]) {
      if (d.type === 'campfire') {
        ...
        d.next = now + 500;
        const heal = DEFENSES.campfire.heal * 0.5;
        for (const q of [...room.players, ...room.survivors.list.values()]) {
          ...
          const max = q.maxHp ?? 100;
          if (q.hp < max) { q.hp = Math.min(max, q.hp + heal * (q.maxHp ? 3 : 1)); if (!q.isSurvivor) room.sendInv(q); }
        }
        continue;
      }
      ...
      if (d.type === 'spikes' || d.type === 'darts' || d.type === 'flame') {
        ...
        const eye = [d.pos[0], d.pos[1] + 1.2, d.pos[2]];
        ...
        d.next = now + 1000 / def.rate;
        const aim = [best.pos[0], best.pos[1] + 1.1 * best.s, best.pos[2]];
        if (d.type === 'turret') room.damageZombie(best, def.dmg * (room.buffActive('damage') ? 1.3 : 1), this.owner(d), 'turret');
        else {
          const dir = aim.map((v, j) => v - eye[j]), l = Math.hypot(...dir);
          room.combat.rocket([...eye], dir.map(v => v / l), this.owner(d), def.dmg, def.splash);
        }
