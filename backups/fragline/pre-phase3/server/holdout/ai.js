...
function step(room, z, dt, now, hash) {
  ...
  if (z.state === ZS.MOVE) {
    ...
    if (!attack && !face && z.stuck > 0.8 && goal) { // blocked by a piece right in front: hit it
  ...
  const speed = z.state === ZS.MOVE ? t.speed * (now < z.slowUntil ? 1 - z.slow : 1) : 0;
...
function strike(room, z) {
  const a = z.target, t = z.t, mul = room.director.dmgMul;
  z.target = null;
  if (!a) return;
  if (a.kind === 'p') {
    const p = a.ref, pp = p.st.p, dh = Math.hypot(pp[0] - z.pos[0], pp[2] - z.pos[2]), dy = pp[1] - z.pos[1];
    if (p.alive && !p.downed && dh <= t.reach + 0.5 && dy > -1.2 && dy < 1.8 * z.s) room.hurtPlayer(p, t.dmg * mul, z);
  } else if (a.kind === 's') {
...
function launch(room, z) {
  ...
  room.addProjectile(o, [dx / T, dy / T + 0.5 * GLOB_GRAVITY * T, dz / T], z);
}
