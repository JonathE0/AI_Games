...
import { ZTYPES, WAVES, composeWave, waveBudget, laneCount, hpMult, dmgMult, aliveCap, countMult } from '../../shared/zombies.js';
import { SKY } from '../../shared/skyboss.js';

const SKY_WAVE_HORDE = 0.5; // the Colossus wave sends a lighter ground horde without Brutes (its runners make up the rest)
...
export class Director {
  ...
  plan(w) {
    ...
    this.planned = { wave: w, lanes: ids.slice(0, count) };
  ...
  start(w, now = Date.now()) {
    ...
    Object.assign(this, { wave: w, n, lanes: this.planned.lanes, lastLanes: this.planned.lanes, hpMul: hpMult(n, w), dmgMul: dmgMult(w) });
    const budget = waveBudget(w, n, this.room.diff) * (w === SKY.wave ? SKY_WAVE_HORDE : 1);
    this.queue = this.shuffle(composeWave(w, n, this.room.diff, this.rng, budget).filter(id => w !== SKY.wave || id !== 'brute'));
    if (w === WAVES) this.queue.splice(Math.floor(this.queue.length * 0.7), 0, 'alpha'); // pop() order: arrives ~30 % in
  ...
  rescale(n) {
    ...
    if (ratio > 1 && left > 0) {
      this.queue.push(...composeWave(this.wave, n, 1, this.rng, left * (ratio - 1)));
  ...
  done() { return !this.queue.length && this.room.zombies.size === 0 && !this.room.sky?.alive; }
}
