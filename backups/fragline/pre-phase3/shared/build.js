...
export function pieceBox(p) {
  ...
  if (p.kind === 'wall') {
    ...
    b = { min: [x, y, z], max: [x + C, y + T, z + C] };
  } else {
    const r = RAMP_DIRS[p.o];
    b = { min: [x, y, z], max: [x + C, y + H, z + C], ramp: { axis: r.axis, dir: r.dir, slope: H / C } };
  }
...
export function checkPlacement(p, w) {
  ...
  for (const s of w.statics) if (s.mat !== 'f' && overlaps(b, s, 0.02)) return 'Blocked';
  for (const s of w.nodes) if (overlaps(b, s, 0.02)) return 'Blocked';
