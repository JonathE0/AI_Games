...
test('backpack: five gun slots, typed ammo bought at the Core, reloads draw from the backpack', () => {
  const room = new HoldoutRoom('B', {});
  const a = join(room, 'A'), p = a.player;
  assert.equal(p.guns[0].w, 'pistol');
  assert.equal(p.guns.length, 5);
  p.money = 20000;
  room.handle(p, { t: 'buy', item: 'ar' });
  assert.equal(p.guns[1].w, 'ar');
  assert.equal(p.guns[1].r, 1);
  room.handle(p, { t: 'buy', item: 'ar' });
  assert.match(a.last('deny').text, /Already carrying/);
  ...
  room.handle(p, { t: 'reload', uid: p.guns[1].uid, need: 60 });
  assert.equal(a.last('grant').n, Math.min(60, med + AMMO.medium.pack));
  for (const id of ['smg', 'pump', 'h_ssg']) room.handle(p, { t: 'buy', item: id });
  assert.ok(p.guns.every(Boolean));
  room.handle(p, { t: 'buy', item: 'rocket', slot: 2 }); // full: swaps with the chosen slot, old gun drops
  assert.equal(p.guns[2].w, 'rocket');
  const dropped = [...room.inventory.pickups.values()].find(pk => pk.kind === 'gun');
  assert.equal(dropped.w, 'smg');
  p.st.p = [...dropped.pos];
  room.handle(p, { t: 'pickup', id: dropped.id, slot: 0 });
  assert.equal(p.guns[0].w, 'smg');
  assert.ok([...room.inventory.pickups.values()].some(pk => pk.w === 'pistol'));
  p.st.p = [30, 0, 30];
...
test('grenades, molotovs and freeze grenades hurt only zombies and credit the thrower', () => {
  const room = new HoldoutRoom('G', {});
  const a = join(room, 'A'), p = a.player;
  room.startWave(1); room.director.queue = [];
  p.items.grenade = 2; p.items.freeze = 1; p.items.molotov = 1;
  p.st.p = [0, 0, -6];
...
test('traps and turrets defend on their own; campfires heal', () => {
  const room = new HoldoutRoom('D', {});
  const a = join(room, 'A'), p = a.player;
  p.st.p = [0, 0, -6];
  room.handle(p, { t: 'build', kind: 'floor', ...tile(-4, -12), l: 0, mat: 'wood' });
  T += 500;
  room.handle(p, { t: 'build', kind: 'wall', ...tile(4, -12), l: 0, o: 1, mat: 'wood' });
  const [floor, wall] = [...room.pieces.values()];
  p.items.spikes = 1; p.items.darts = 1; p.items.campfire = 1;
  ...
  const r2 = new HoldoutRoom('T', {}), q = join(r2, 'Q').player;
  q.st.p = [-6, 0, -3];
  q.items.turret = 1;
...
test('the Colossus: only snipers hurt its weak points, and the wave waits for it', () => {
  ...
  p.guns[1] = { w: 'h_awp', uid: 500, r: 0 };
  p.guns[2] = { w: 'ar', uid: 501, r: 0 };
  p.st.p = [0, 0, 0];
  const eye = [0, 1.6, 0];
