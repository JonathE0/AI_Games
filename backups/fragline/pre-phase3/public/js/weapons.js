...
// Zombie Holdout adds five gun slots (knife = slot 0), backpack ammo (reloads ask the server for rounds),
// burst rifles, a spinning minigun, the rocket launcher, and sniper shots at the Colossus' weak points.
...
export class Weapons {
  ...
  // holdout = true: slots 1–5 are guns, 0 is the knife (harvest tool), reserve ammo lives in `pool`
  configure(holdout) {
    this.pooled = holdout;
    this.slots = holdout ? { 0: KNIFE, 1: null, 2: null, 3: null, 4: null, 5: null } : { 1: null, 2: null, 3: KNIFE };
    this.order = holdout ? [1, 2, 3, 4, 5, 0] : [1, 2, 3];
  ...
  // Server inventory update: { slot: item | null }. New uids = freshly bought/looted guns (full or given mag).
  setInventory(items) {
    let fresh = 0;
    for (const [k, it] of Object.entries(items)) {
      const slot = +k, prev = this.slots[slot];
      this.slots[slot] = it || null;
      if (it && !(it.uid in this.ammo)) this.ammo[it.uid] = { mag: it.mag ?? WEAPONS[it.w].mag, reserve: WEAPONS[it.w].reserve, granted: 0 };
      if (it && prev?.uid !== it.uid && !fresh) fresh = slot;
    }
    ...
    else if (!this.slots[this.slot]) this.equip(this.order.find(s => this.slots[s]) ?? this.knifeSlot, true);
  }
  ...
  reload() {
    ...
    if (!a || w.cat === 'melee' || this.reloadEnd || a.mag >= w.mag || now < this.deployEnd) return;
    if (this.pooled) {
      if (a.granted <= 0 && !(this.pool[w.ammo] > 0)) { this.g.holdout?.noAmmo(w); return; }
      const need = w.mag - a.mag - a.granted;
  ...
  update(dt, input) {
    ...
    if (this.reloadEnd && !w.shellReload && now >= this.reloadEnd) {
      if (this.pooled && a.pending && now < this.reloadEnd + 1.5) { /* waiting for the backpack */ }
        ...
        const take = Math.min(w.mag - a.mag, this.pooled ? a.granted : a.reserve);
        a.mag += take;
    ...
    if (w.shellReload && this.reloadEnd && now >= this.shellNext) {
      ...
      } else if (this.pooled && a.pending && a.mag < w.mag) this.shellNext = now + 0.1;
      const left = this.pooled ? a.granted + (a.pending ? 1 : 0) : a.reserve;
      if (a.mag >= w.mag || left <= 0) { this.reloadEnd = 0; g.sound.play('rl_pump', { vol: 0.9, delay: 0.05 }); }
    }
  ...
  fire() {
    ...
    this.lastShot = now;
    if (w.projectile === 'rocket') { // the server flies the rocket and tells everyone
      g.net.send({ t: 'rocket', uid: this.cur.uid, o: eye.map(r3), d: dirFromAngles(baseYaw, basePitch).map(r3) });
      this.punch[1] += kick[1];
      return;
    }
    const target = g.shotTargets(), sky = w.cat === 'sniper' ? g.holdout?.skyTargets() : null, balloons = g.holdout?.balloonTargets();
    const dirs = [], ends = [], hits = [], skyHits = [], bal = [];
    let heard = false;
    ...
    const msg = { t: 'shot', w: w.id, o: eye.map(r3), d: dirs, e: ends, h: hits };
