...
import { GRID, BMATS, MAT_IDS, KINDS, PIECE_COST, REACH, EDIT_GRID, pieceBox, pieceBoxes, slotKey, checkPlacement, validMask, doorOf, distToBox } from '/shared/build.js';
import { OUTPOST_NODES, NODE_TYPES } from '/shared/outpost.js';
import { ITEMS, TRAPS } from '/shared/holdout.js';
...
function pieceGeometry(kind) {
  ...
  else g = rampGeometry({ min: [-C / 2, 0, -C / 2], max: [C / 2, H, C / 2], ramp: { axis: 0, dir: 1, slope: H / C } });
  worldUV(g, 1.2);
  return g;
}
...
export function pieceTransform(p, out) {
  ...
  } else if (p.kind === 'floor') _v.set(x0 + C / 2, y0 + T / 2, z0 + C / 2);
...
export class BuildMode {
  constructor(game, holdout) {
    ...
    this.h = holdout;
    this.active = false;
    this.kind = 'wall';     // wall | floor | ramp | trap
    this.mat = 'wood';
    this.trap = null;       // selected trap item id
    this.turn = 0;
    this.slot = null;
    this.valid = false;
    this.reason = '';
  ...
  select(kind) {
    if (kind === 'trap') {
      const owned = TRAPS.filter(id => this.h.items[id] > 0);
      if (!owned.length) { this.h.say('No traps or turrets — buy them at the Core'); return; }
      if (!owned.includes(this.trap)) this.trap = owned[0];
    } else if (this.active && this.kind === kind && kind === 'ramp') this.rotate();
    this.kind = kind;
    this.toggle(true);
  }

  // wheel: materials for pieces, trap type when placing traps
  cycle(dir) {
    if (this.kind === 'trap') {
      const owned = TRAPS.filter(id => this.h.items[id] > 0);
      if (owned.length) this.trap = owned[(owned.indexOf(this.trap) + dir + owned.length) % owned.length];
    } else {
  ...
  rotate() { this.turn = (this.turn + 1) % 4; this.g.sound.play('tick', { vol: 0.4 }); }
  ...
  // Where the selected trap would go: { pid, side, i, k, box (for the ghost), reason }
  aimTrap() {
    const it = ITEMS[this.trap], { eye, dir, hit } = this.aim();
  ...
  update(dt, firing) {
    ...
    if (!show) return;
    if (this.kind === 'trap') {
      if (!(this.h.items[this.trap] > 0)) { const owned = TRAPS.filter(id => this.h.items[id] > 0); this.trap = owned[0] ?? null; }
      const t = this.slot = this.trap ? this.aimTrap() : { reason: 'No traps left' };
  ...
  tryPlace() {
    this.update(0, false);
    if (this.kind === 'trap') {
      if (this.valid) { const t = this.slot; this.g.net.send({ t: 'place', item: this.trap, pid: t.pid, side: t.side, i: t.i, k: t.k }); this.g.sound.play('build', { vol: 0.5 }); }
  ...
  costText() { return this.kind === 'trap' ? `${ITEMS[this.trap]?.name ?? 'No traps'} ×${this.h.items[this.trap] || 0}` : `${BMATS[this.mat].name.toUpperCase()} ${PIECE_COST}`; }
