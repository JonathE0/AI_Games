...
export class Game {
  ...
  enterGame(m) {
    ...
    $('roomInfo').innerHTML = this.vsBot ? 'Practice match vs bot'
      : `${this.mode.name}${this.holdout ? ` · ${m.diff} · up to ${this.mode.maxPlayers} players` : ''}<br>Room code <b>${this.code}</b><br>Invite link: <b>${local ? `http://&lt;your-ip&gt;:${location.port || 80}` : location.origin}/#${this.code}</b>`;
  ...
  setVCursor(x, y) {
    ...
    const b = document.elementFromPoint(...this.vcur)?.closest('#buyMenu button, #bagMenu button');
    if (b && !b.disabled) { b.classList.add('hover'); this.hoverEl = b; }
  }

  vClick() {
    const b = this.vcur && document.elementFromPoint(...this.vcur)?.closest('#buyMenu button, #bagMenu button');
  ...
  onInv(m) {
    ...
    this.weapons.setInventory(this.holdout ? Object.fromEntries((m.guns || []).map((g, i) => [i + 1, g])) : { 1: m.s1, 2: m.s2 });
  ...
  update(dt) {
    ...
    } else if ((this.ui === 'buy' || this.ui === 'bag') && this.vcur && (mx || my)) {
    ...
    const moveFree = inp.locked && !this.intro && (!this.ui || this.ui === 'buy' || this.ui === 'bag');
    ...
    const maxSp = downed ? 3.5 : this.weapons.speedLimit() * (this.holdout?.carrying ? 0.75 : 1); // downed: x crouch speed = a slow crawl
