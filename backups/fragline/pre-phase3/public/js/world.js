...
export class World {
  ...
  setDusk(k) {
    ...
    this.duskK = k;
    const day = new THREE.Color(0xb3cde0), dusk = new THREE.Color(0x3b3552);
    this.scene.background.copy(day).lerp(dusk, k);
    this.scene.fog.color.copy(this.scene.background);
    this.scene.fog.near = 70 - 30 * k;
    this.scene.fog.far = 160 - 60 * k;
    this.hemi.intensity = 1.6 - 0.75 * k;
    this.hemi.color.setHex(0xeef6ff).lerp(new THREE.Color(0x9c8cff), k);
    this.sun.intensity = 2.2 - 1.3 * k;
    this.sun.color.setHex(0xfff2de).lerp(new THREE.Color(0xff9a6b), k);
  }
