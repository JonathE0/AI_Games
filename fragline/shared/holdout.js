...
export const ITEMS = {
  ...
  turret: { name: 'Auto Turret', kind: 'deploy', mount: 'ground', price: 2250, max: 3 },
  rturret: { name: 'Rocket Turret', kind: 'deploy', mount: 'ground', price: 3750, max: 2 },
  campfire: { name: 'Campfire', kind: 'deploy', mount: 'ground', price: 400, max: 4 },
};
...
export const ATTACH = {
  ...
  flashlight: { name: 'Flashlight', slot: 'rail', price: 500, desc: 'Light your way at night (L)' },
...
const ammoFor = (id, n = 1) => (WEAPONS[id].ammo ? { kind: 'ammo', type: WEAPONS[id].ammo, n: AMMO[WEAPONS[id].ammo].pack * n } : null);
...
export function rollLoot(kind, rng = Math.random) {
  ...
  if (kind === 'chest') {
    ...
    out.push(lootGun(rng, w, [0, 10, 40, 35, 15], 0.4, 0.3), ammoFor(w, 2), anyItem(rng), anyItem(rng), mats(rng, 100));
...
export const SMITH = {
  ...
  turret: {
    dmg: { name: '+25% damage', price: 800, max: 2 },
    range: { name: '+20% range', price: 600, max: 2 },
    rate: { name: '+20% fire rate', price: 800, max: 2 },
    ammo: { name: 'Refill ammo', price: 400 },
    inc: { name: 'Incendiary rounds', price: 1000, max: 1 },
    frost: { name: 'Frost rounds', price: 1000, max: 1 },
    plate: { name: 'Armor plate +300 HP', price: 700, max: 1 },
  },
};

// ---------- player classes (picked in the lobby or at the Core during a break) ----------
export const CLASSES = {
  tank: { name: 'Tank', hp: 150, speed: 0.88, desc: '150 health · a bit slower' },
  assault: { name: 'Assault', hp: 100, speed: 1.1, dmg: 1.2, mag: 1.5, ammo: 1.5, desc: '+20% damage · 50% bigger magazines · carries 50% more ammo · faster' },
  medic: { name: 'Medic', hp: 100, speed: 1, regen: 3, aura: 4, auraR: 5, revive: 0.5, desc: 'Heals over time and heals people near you · revives twice as fast · free bandages and a medkit every wave' },
};
