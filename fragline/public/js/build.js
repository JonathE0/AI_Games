...
import { GRID, BMATS, MAT_IDS, KINDS, PIECE_COST, REACH, EDIT_GRID, RAMP_THICK, pieceBox, pieceBoxes, slotKey, checkPlacement, validMask, doorOf, distToBox } from '/shared/build.js';
...
export class BuildMode {
  ...
  select(kind) {
    if (kind === 'trap' || kind === 'deploy') {
      ...
      if (!owned.length) { this.h.say(kind === 'trap' ? 'No traps — buy spikes, wall darts or flame grills at the Core' : 'No turrets or campfires — buy them at the Core'); return; }
  ...
  update(dt, firing) {
    ...
    if (this.placing) {
      ...
      const t = this.slot = this.item ? this.aimTrap() : { reason: this.kind === 'trap' ? 'No traps left' : 'No turrets or campfires left' };
