...
const LOOK = {
  ...
  swooper: { skin: [0x3a3036], shirt: [0x2a2228], pants: 0x1a1418, eye: 0xff9a2a, flyer: true },
...
const _burn = new THREE.Color(0xff6a1a), _soak = new THREE.Color(0x2f7fff), _chill = new THREE.Color(0xaef4ff), _ice = new THREE.Color(0xd8fbff);
...
export class ZombieView {
  ...
  paint(zb) {
    ...
    const cols = [L.pants, L.pants, zb.shirt, zb.shirt, zb.shirt, zb.shirt, zb.skin, zb.skin, zb.skin, L.helmet ?? L.sac ?? L.accColor ?? zb.skin, zb.skin];
    const k = zb.flash, fx = zb.fx || 0;
    // status tints: burning orange, soaked blue, chilled / frozen icy
    const tint = zb.st === 4 ? _ice : fx & 4 ? _chill : fx & 1 || L.hot ? _burn : fx & 2 ? _soak : null;
    cols.forEach((c, i) => { _c.setHex(c); if (tint) _c.lerp(tint, zb.st === 4 ? 0.65 : 0.4); this.body.setColorAt(base + i, _c.lerp(_w, k * 0.7)); });
  ...
  update(dt, now) {
    ...
    for (const zb of [...this.list.values()]) {
      if (zb.dead) {
        ...
        const st = b.st;
        if (st !== zb.st) {
          if (st === 4 || zb.st === 4) { zb.st = st; this.paint(zb); } // frozen solid / thawed
  ...
  // Swoopers: body flat out, arms stretched into flapping wings, legs trailing.
  poseFlyer(zb, dt) {
    const sc = zb.s, [x, y, z] = zb.pos, base = zb.slot * SLOTS, M = this.body;
    zb.phase += dt * (zb.st === 1 ? 22 : 11);
    const flap = zb.dead ? 0 : Math.sin(zb.phase) * (zb.st === 2 ? 0.15 : 0.7);
    const dive = zb.st === 2 ? 0.5 : zb.st === 1 ? -0.3 : 0;
    const fall = zb.dead ? Math.min(1, zb.deadT / 0.6) : 0;
    _e.set(-Math.PI / 2 + 0.25 + dive + fall * 1.2, zb.yaw, 0, 'YXZ');
    _root.compose(_v.set(x, y - (zb.dead ? zb.deadT * 6 : 0) - 0.9 * sc, z), _q.setFromEuler(_e), _one.set(sc, sc, sc));
    const hb = HB;
    for (let i = 0; i < 9; i++) {
      const h = hb[i];
      if (i >= 4 && i <= 7) continue;
      M.setMatrixAt(base + i, part(_m, _root, 0, 0, 0, 0, h.c[0], h.c[1], h.c[2], h.h[0] * 2, h.h[1] * 2, h.h[2] * 2));
    }
    const sy = hb[4].c[1] + hb[4].h[1] * 0.6;
    for (let s = -1; s <= 1; s += 2) { // wings: a long thin panel per side, pivoting at the shoulder
      _m.copy(_root).multiply(_t.makeTranslation(s * 0.2, sy, 0)).multiply(_r.makeRotationZ(s * flap)).multiply(_t.makeTranslation(s * 0.55, 0, 0.05)).multiply(_s.makeScale(1.0, 0.04, 0.42));
      M.setMatrixAt(base + (s < 0 ? 4 : 5), _m);
      M.setMatrixAt(base + (s < 0 ? 6 : 7), ZERO);
    }
    M.setMatrixAt(base + 9, ZERO);
    M.setMatrixAt(base + 10, ZERO);
    const hd = hb[8];
    for (let i = 0; i < 2; i++) this.eyes.setMatrixAt(zb.slot * 2 + i, part(_m, _root, 0, 0, 0, 0, (i ? 0.05 : -0.05), hd.c[1] + 0.04, hd.c[2] - hd.h[2] - 0.006, 0.06, 0.04, 0.02));
    this.blobs.setMatrixAt(zb.slot, _m.compose(_v.set(x, 0.02, z), _q.identity(), _one.set(sc * 0.6, 1, sc * 0.6)));
  }

  pose(zb, dt) {
    if (zb.look.flyer) return this.poseFlyer(zb, dt);
