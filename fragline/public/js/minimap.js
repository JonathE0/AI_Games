...
export class Minimap {
  ...
  zombieVisible(zb, me, dark, torch, yaw) {
    if (!dark) return true;
  ...
  draw(big) {
    ...
    // zombies (bosses bigger, flyers orange)
    for (const zb of h.zombies.list.values()) {
      if (zb.dead || !this.zombieVisible(zb, me, dark, h.flashOn, pl.yaw)) continue;
      dot(zb.pos[0], zb.pos[2], zb.t.boss ? 5 : zb.t.flyer ? 2.2 : 2, zb.t.boss ? '#ff2a2a' : zb.t.flyer ? '#ff9a2a' : '#e04848');
    }
