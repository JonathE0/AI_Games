...
export class FlowField {
  ...
  compute() {
    const { nx, nz, solid, cost, dist, next } = this;
    dist.fill(Infinity);
    next.fill(-1);
    const heap = new Heap();
  ...
  step(x, z) {
    let n = this.idx(x, z);
    if (n < 0 || !isFinite(this.dist[n])) n = this.nearestReachable(x, z);
    if (n < 0) return null;
    const m = this.next[n];
  ...
  nearestReachable(x, z) {
    ...
    for (let r = 1; r < 6; r++) for (let di = -r; di <= r; di++) for (let dk = -r; dk <= r; dk++) {
      ...
      const n = i + k * this.nx;
      if (isFinite(this.dist[n])) return n;
    }
    return -1;
  }
