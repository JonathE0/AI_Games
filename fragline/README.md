...
- **Classes** (B at the Core, in the lobby or during a break):
  ...
  | Class | Effect |
  | --- | --- |
  | Tank | 150 HP, a bit slower |
  | Assault | +20 % damage, 50 % bigger magazines, carries 50 % more ammo, 10 % faster |
  | Medic | regenerates, heals teammates and survivors within 5 m, revives twice as fast, free bandages + medkit every wave |
...
- **Zombies:** Shambler, Runner, Spitter (lobs acid over walls), Brute (armored wall-breaker), plus:
  ...
  ink — 7), Swooper (small flyer that dives at you over your walls — 8), Burrower (tunnels once under a build and
...
- **Bosses** every 5 waves, each repeat 50 % tougher:
  ...
  - **The Brood Titan (10, 25…):** announced at the start of the wave, stomps in once the horde is dead. Its
    riders throw acid and can't be hurt while mounted; every 18 s some leap down. Kill the Titan and the rest fall
    off. The first one unlocks **the Blacksmith**.
  - **The Maw (15, 30…):** a giant worm hunting underground and bursting up under builds and players. Hold **E**
    ...
    thumpers or blast its mouth to stop it. Drops a tier III elemental gun.
- **Night waves** (a quarter of the time from wave 4, never with a boss; announced a break ahead): nearly black,
  short view distance. Find zombies by their eyes, muzzle flashes and sound — or fit a **flashlight** (**L**).
  Zombies drop flashlights on night waves.
- **Inventory (I):** Minecraft-style — 6 hotbar slots (**1–6**, **F** = harvesting knife) + 18 backpack slots
  ...
  item outside the panel to drop it. Ammo and materials are counters. Next to the team chest the chest opens
...
- **Guns:** Fortnite-style pistols, SMGs, ARs, shotguns, a Hand Cannon, rockets, the SSG 08 and AWP, plus the
  **grenade launcher** (impact grenades, explosive ammo), the **Shockwave Blaster** (crowd control: shoves
  zombies back by their weight — runners fly, brutes budge, bosses don't) and the **Slasher Blade** (Stalker
  ...
  any shop gun at the Core, find one, or infuse at the Blacksmith): fire (burns), water (soaks
...
- **Building (G):** walls, floors and stairs on a 4 m grid with their own keys while build mode is on —
  **Q** wall · **E** stair · **F** floor · **T** trap · **Z** turret/campfire · **R** rotate the stair (it
  ...
  **X** demolish your own piece, hold **E** to repair. Rebind them in Settings → Key bindings → *Building*.
  ...
  below. **Structural integrity:** anything that loses its connection to the ground collapses.
...
- **Shop (B, inside the ring around the Core):** guns, ammo, grenades / molotovs / freeze grenades,
  bandages / medkits / shield potions (**H**), armor, and separate **Traps** (floor spikes, wall darts, flame
  grills) and **Turrets & Deployables** (auto turret $2250, rocket turret $3750, campfire) sections. Team
  powerups (overshield, +30 % damage, +30 % fire rate, fortify every build, Core barrier) can be paid from the
  team bank.
- **Team chest** (next to the Core): share money, materials, ammo and items; pooled money is the team bank.
  Hold **E** at the Core to heal it (one player at a time).
...
- **Supply drops and chests:** balloons float crates down during waves (shoot the balloons to drop them fast);
  chests around the map restock every 3 waves.
- **Survivors (rescue waves 3, 7, 11…):** carry the wounded from the corner shelters into the Core ring (+$500).
  They fight by tier — Recruit (pistol), Guard (SMG), Soldier (AR), Marksman (DMR) — never heal on their own
  (use a bandage or medkit on them with **H**, a Medic or a campfire) and die for good.
